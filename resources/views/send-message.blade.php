...
<body>
    ...
    <form id="send-message-form" action="{{ route('messages.store') }}" method="post">
        @csrf
        <div>
    ...
    `.replace(/':id'/g, message.id);
            ...
            $(document).on('click', '.edit-message', function() {
                ...
                window.location.href = "{{ route('messages.edit', ':id') }}".replace(':id', messageId);
            });
            ...
            $(document).on('submit', '#edit-message-form', function(e) {
                ...
                $.ajax({
                    url: "{{ route('messages.update', ':id') }}".replace(':id', messageId),
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        _method: 'PUT',
            ...
            $(document).on('submit', '.delete-message-form', function(e) {
                ...
                $.ajax({
                    url: "{{ route('messages.destroy', ':id') }}".replace(':id', messageId),
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        _method: 'DELETE'
